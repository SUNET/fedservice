...
class TestRpService(object):
    ...
    def create_trust_chains(self):
        ...
        _fed_context = self.rp['federation_entity'].context
        save_trust_chains(_fed_context, _trust_chains)
    ...
    def test_create_reqistration_request(self):
        ...
        _trust_chain = _jwt.jwt.headers.get("trust_chain")
        _peer_trust_chain = _jwt.jwt.headers.get("peer_trust_chain")

        _tc = verify_trust_chain(self.oas, _trust_chain)[0]
        _ptc = verify_trust_chain(self.oas, _peer_trust_chain)[0]
